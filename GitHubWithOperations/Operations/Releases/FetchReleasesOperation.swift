...
class FetchReleasesOperation: BaseOperation {
    ...
    init(repo: Repo) {
        ...
        super.init()
        self.name = "Fetch releases for repo operation"
...
extension FetchReleasesOperation: NetworkRequestable {
    
    public var requestURL: URL? {
        guard token != nil else {
            return nil
        }
        return URL(string: "\(GitHubConfiguration().baseURL)/repos/\(UserAuthenticationManager.shared.username ?? "")/\(self.repoName)/releases")
    }

...
class FetchUserReposOperation: BaseOperation {
    ...
    override init() {
        super.init()
        self.name = "Fetch user repos operation"
...
extension FetchUserReposOperation: NetworkRequestable {
    
    public var requestURL: URL? {
        guard token != nil else {
            return nil
        }
        return URL(string: "\(GitHubConfiguration().baseURL)/users/\(UserAuthenticationManager.shared.username ?? "")/repos")
    }

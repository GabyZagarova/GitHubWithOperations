//
//  FetchPullRequestsOperation.swift
...
class FetchPullRequestsOperation: BaseOperation {
    ...
    init(repo: Repo) {
        ...
        super.init()
        self.name = "Fetch pull requests for repo operation"
...
extension FetchPullRequestsOperation: NetworkRequestable {
    
    public var requestURL: URL? {
        guard token != nil else {
            return nil
        }
        return URL(string: "\(GitHubConfiguration().baseURL)/repos/\(UserAuthenticationManager.shared.username ?? "")/\(self.repoName)/pulls")
    }
